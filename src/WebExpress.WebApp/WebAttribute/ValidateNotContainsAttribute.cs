...
namespace WebExpress.WebApp.WebAttribute
{
    ...
    public sealed class ValidateNotContainsAttribute : Attribute, IValidation
    {
        ...
        /// The substring that must not appear in the validated string. 
        /// </param>
        /// <param name="message">
        /// The error message that will be returned when validation fails.
        /// </param>
        public ValidateNotContainsAttribute(string substring, string message = null)
        {
            Substring = substring;
            Message = message ?? "webexpress.webapp:validation.notcontains";
        }
