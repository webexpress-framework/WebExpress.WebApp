...
{
    /// <summary>
    /// The fluent C# authoring surface for the resources of a ViewState.
    /// It complements the state and service authoring of
    /// <see cref="DataAuthoringExtensions"/> with the resource declaration, so a
    /// ViewState is authored as one chain of State, Service and Resource calls. The
    ...
    {
        /// <summary>
        /// Declares a named resource of the ViewState, emitted as a wx-resource island.
        /// The resource binds the ViewState state to a ViewState service, so the ViewState
        /// loads it centrally and re-queries it when a control changes the state it
        /// depends on. Each call adds one resource.
        /// </summary>
        /// <typeparam name="T">The ViewState host control type.</typeparam>
        /// <param name="control">The ViewState host control.</param>
